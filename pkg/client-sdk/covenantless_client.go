...
func NewBitcoinReceiversFromJSON(receivers string) ([]Receiver, error) {
	var recs []bitcoinReceiver
	if err := json.Unmarshal([]byte(receivers), &recs); err != nil {
		return nil, err
	}

	result := make([]Receiver, len(recs))
	for i, r := range recs {
		result[i] = r
	}
...
func LoadCovenantlessClient(storeSvc store.ConfigStore) (ArkClient, error) {
	if storeSvc == nil {
		return nil, fmt.Errorf("missing store service")
	}
