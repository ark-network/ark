...
import (
	"context"
	application "github.com/ark-network/ark/server/pkg/ark-wallet-daemon/internal/core"
...
func (h *WalletServiceHandler) WatchScripts(
	ctx context.Context, request *walletv1.WatchScriptsRequest,
) (*walletv1.WatchScriptsResponse, error) {
	if err := h.walletSvc.WatchScripts(ctx, request.Scripts); err != nil {
		return nil, err
	}
	return &walletv1.WatchScriptsResponse{}, nil
}

func (h *WalletServiceHandler) UnwatchScripts(
	ctx context.Context, request *walletv1.UnwatchScriptsRequest,
) (*walletv1.UnwatchScriptsResponse, error) {
	if err := h.walletSvc.UnwatchScripts(ctx, request.Scripts); err != nil {
		return nil, err
	}
	return &walletv1.UnwatchScriptsResponse{}, nil
}
...
func (h *WalletServiceHandler) IsTransactionConfirmed(
	ctx context.Context, req *walletv1.IsTransactionConfirmedRequest,
) (*walletv1.IsTransactionConfirmedResponse, error) {
	confirmed, blocknumber, blocktime, err := h.walletSvc.IsTransactionConfirmed(ctx, req.GetTxid())
	if err != nil {
		return nil, err
	}
	return &walletv1.IsTransactionConfirmedResponse{
		Confirmed:   confirmed,
...
func (h *WalletServiceHandler) GetSyncedUpdate(
	_ *walletv1.GetSyncedUpdateRequest, stream walletv1.WalletService_GetSyncedUpdateServer,
) error {
	...
	case <-ch:
		return stream.Send(&walletv1.GetSyncedUpdateResponse{})
	}
}

// NotificationStream streams notifications to the client.
func (h *WalletServiceHandler) NotificationStream(
	_ *walletv1.NotificationStreamRequest, stream walletv1.WalletService_NotificationStreamServer,
) error {
	ctx := stream.Context()
	...
	for {
		...
		case notification, ok := <-ch:
			...
			entries := make([]*walletv1.VtoxsPerScript, 0, len(notification))
			for script, vtxos := range notification {
				entry := &walletv1.VtoxsPerScript{
					Script: script,
					Vtxos:  make([]*walletv1.VtxoWithKey, 0, len(vtxos)),
				}
				for _, v := range vtxos {
					entry.Vtxos = append(entry.Vtxos, &walletv1.VtxoWithKey{
						Txid:  v.Key.Txid,
						Vout:  v.Key.VOut,
						Value: v.Value,
					})
			...
			resp := &walletv1.NotificationStreamResponse{
				Entries: entries,
			}
